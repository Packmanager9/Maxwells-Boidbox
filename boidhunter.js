...
window.addEventListener('DOMContentLoaded', (event) => {
    ...
    class Circle {
        ...
        frictiveMove() {
            if (this.reflect == 1) {
                ...
                if (this.y + this.radius > canvas.height * .9) {
                    if (this.ymom > 0) {
                        this.ymom *= -.5
                    }
                }
    ...
    class Boid {
        constructor() {
            ...
            this.flapping = 100
            this.flap = 0
            this.leftwing = new Circle(0, 0, 1.5, this.body.color)
            this.rightwing = new Circle(0, 0, 1.5, this.body.color)
            this.dis = this.body.radius + this.leftwing.radius
            let fuzz = Math.random() * Math.PI * 2
            this.angle1 = -2 - fuzz
            this.angle2 = -1 + fuzz
            this.state = 0
        ...
        draw() {
            ...
            if (this.dead == 0) {
                ...
                if (count > 0) {
                    ...
                    avgvecx /= count
                    avgvecy /= count
                    this.body.xmom += (avgposx - this.body.x) / this.tog
                    this.body.ymom += (avgposy - this.body.y) / this.tog

                    this.body.xmom = ((this.body.xmom * (this.coh - 1)) + avgvecx) * (1 / (this.coh))
                    this.body.ymom = ((this.body.ymom * (this.coh - 1)) + avgvecy) * (1 / (this.coh))
                }
            ...
            if (Math.abs(this.body.xmom) + Math.abs(this.body.ymom) > max) {
                max = Math.abs(this.body.xmom) + Math.abs(this.body.ymom)
                console.log(max)
            }


            this.body.frictiveMove()
            this.leftwing.x = this.body.x + (Math.cos(this.angle1) * this.dis)
            this.leftwing.y = this.body.y + (Math.sin(this.angle1) * this.dis)
            this.rightwing.x = this.body.x + (Math.cos(this.angle2) * this.dis)
            this.rightwing.y = this.body.y + (Math.sin(this.angle2) * this.dis)
        }
        rdraw() {
    ...
    let level = 0
    let gooon = 0
    let goon = new Rectangle(300, 300, 100,100, "white")
    let walls = []
    let wall1 = new Rectangle((700 / 3 - 10), 0, 10, 700, "black")
    walls.push(wall1)
    let wall2 = new Rectangle(700 / 1.5, 0, 10, 700, "black")
    walls.push(wall2)
    let boxes = []
    let box1 = new Rectangle(0,  0, (700 / 3), 700, "black")
    boxes.push(box1)
    let box2 = new Rectangle((700 / 3),  0, (700 / 3), 700, "black")
    boxes.push(box2)
    let box3 = new Rectangle((700 / 3)*2,  0, (700 / 3), 700, "black")
    boxes.push(box3)
